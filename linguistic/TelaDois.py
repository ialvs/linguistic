...
def telaDois(): # definição da aparência e elementos da tela
        ...
        layout = [
        [sg.Text("LinguisTic - Análise das variáveis sociais",font=(None,20),text_color='Gold')],
    
        [sg.Text("Escolha uma planilha Excel com dados de informantes ",font=(None,20),text_color='Gold'), sg.In(size=(30, 10), enable_events=True, key="escolher",font=(None,15)), sg.FileBrowse(button_text="Procurar",font=(None,15),size=(16,None),initial_folder='linguistic/tests/planilhas')],

        [sg.Text('Série:',font=(None,20),text_color='Gold'),
        sg.Radio('1º ano',"Anos",default=False,key='primeiro_ano',font=(None,25),text_color='Gold'),
        sg.Radio('2º ano',"Anos",default=False,key='segundo_ano',font=(None,25),text_color='Gold'),
        sg.Radio('3º ano',"Anos",default=False,key='terceiro_ano',font=(None,25),text_color='Gold'),
        sg.Radio('4º ano',"Anos",default=False,key='quarto_ano',font=(None,25),text_color='Gold')
        ],

        [sg.Text('Gênero:',font=(None,20),text_color='Gold'),
        sg.Radio('Feminino',"Gênero",default=False,key='feminino',font=(None,20),text_color='Gold'),
        sg.Radio('Masculino',"Gênero",default=False,key='masculino',font=(None,20),text_color='Gold'),
        sg.Text('Outro:',font=(None,20),text_color='Gold'),sg.Input(key='outro',default_text='',font=(None,15),text_color='Gold')
        ],

        [sg.Text('Procedência escolar:',font=(None,20),text_color='Gold'),
        sg.Radio('Pública',"PE",default=False,key='publica',font=(None,20),text_color='Gold'),
        sg.Radio('Privada',"PE",default=False,key='privada',font=(None,20),text_color='Gold'),
        sg.Radio('Mista Pública',"PE",default=False,key='mista_publica',font=(None,20),text_color='Gold'),
        sg.Radio('Mista Privada',"PE",default=False,key='mista_privada',font=(None,20),text_color='Gold')
        ],
        
        [sg.Button("Filtrar", key="filtrar",enable_events=True,disabled=False,font=(None,15),size=(16,None)),
        sg.Button("Sair",key="sair",font=(None,15),size=(16,None))]
        
        
        ]

        return sg.Window('Análise das variáveis sociais', layout, finalize=True,grab_anywhere=True,text_justification='center',element_justification='center')
...
def ver_tabela(dataf): # abre a visualização do dataframe filtrado
    ...
    layout =[
        [sg.Table(values=data, text_color='Gold',
                  headings=cabecalhos,
                  font='Helvetica',
                  key = 'tabela',
        ...
        [sg.Button("Salvar planilha",key='salvar',font=(None,15),size=(16,None)), sg.Input(key='nome_arquivo',default_text='')]

    ]

    return sg.Window('Tabela de dados dos informantes', layout,finalize=True,grab_anywhere=True,resizable=True,element_justification='center')
...
def salvar(valor_input,tela,dataf): # salva o dataframe filtrado
    ...
    if nome != '':
        n_df.to_excel(f'linguistic/saidas/planilhas/{nome}.xlsx',sheet_name='filtrado')
    else:
        n_df.to_excel('linguistic/saidas/planilhas/arquivo_modificado.xlsx',sheet_name='filtrado')
