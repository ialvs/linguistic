...
while True: # abertura do loop pra manter o programa aberto
    window, event, values = sg.read_all_windows() # lê as modificações das telas

    if event == sg.WIN_CLOSED or event == 'sair': # condições para fechamento da tela
        ...
        window2 = telaDois.telaDois() # abre a segunda tela quando o botão for selecionado

    elif event == 'analisar_vl' and not window3:
        window3 = telaTres.terceira_tela() # abre a terceira tela quando o botão for selecionado

    elif event == 'cod_vs' and not window4:
        window4 = telaQuatro.quarta_tela() # abre a quarta tela quando o botão for selecionado

    elif event == 'insert_cod' and not window5:
        window5 = telaCinco.quinta_tela() # abre a quinta tela quando o botão for selecionado

    elif event == "escolher":
        df = telaDois.escolher(values['escolher']) # chama o método responsável pela leitura do dataframe
        ...
        telaDois.salvar(values['nome_arquivo'],ndf_show,n_df) # salva o dataframe como .xlsx

    elif event == 'salvar3':
        telaQuatro.salvar(values['nome_arquivo2'],df_show2,df2) # salva o dataframe como .xlsx
    
    elif event == 'salvar4':
        telaCinco.salvar(values['nome_arquivo3'],window5,df_codl) # salva o dataframe como .xlsx
        ...
        exib = df_codl.to_string(columns=['ocorrências','Código Social'],index=False)
