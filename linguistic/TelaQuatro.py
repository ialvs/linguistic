...
colunas = ['Nome', 'Sobrenome', 'Data de nascimento', 'Faixa etária', 'Gênero',
       'Escolaridade', 'Ensino Superior (indique o semestre)', 'Turma',    
       'Ano de entrada', 'Naturalidade', 'Localidade em que habita',       
       'Procedência escolar', 'Escolaridade do pai', 'Escolaridade da mãe',
       'Naturalidade do pai', 'Naturalidade da mãe', 'Ocupação do pai',
       'Ocupação da mãe', 'Leitura diária de mídias impressas',
       ...
       'Pratica alguma religião? Se sim, qual?', 'Gêneros de leitura habitual',
       'Tempo médio diário de leitura literária ficcional',
       'Gênero literário favorito', 'Autor literário preferido',
       'Uso da escrita', 'Estuda outra língua? Se sim, qual?',
       'Participação em atividades complementares de leitura, escrita e produção textual'] # colunas do dataframe utilizado na pesquisa

simbolos = [' =','-','|','+','F','M','1','2','3','4',
'5','A','N','D','C','c','H','h','>','<',
'"','u','v','U','V','z','Z','_','%','e','E',
'&','g','G','j','J','l','L','#','~','^','}',
'{','!','?','Ç','ç','@','$','[',']',',',':',
';','s','S','y','Y','t','T','r','R','q','Q',
'O','o','k','K','b','B'] # símbolos utilizados para codificar as colunas do dataframe

def quarta_tela(): # definição da aparência e elementos da tela
    layout = [
    [sg.Text("LinguisTic - Codificação das variáveis sociais",font=(None,25),text_color='Gold')],
    [sg.Text("Escolha uma planilha Excel ",font=(None,25),text_color='Gold'), sg.In(size=(25, 1), enable_events=True, key="escolher3",font=(None,15)), sg.FileBrowse(button_text="Procurar",font=(None,15),size=(16,None),initial_folder='linguistic/tests/planilhas')],
    
    
    
    [sg.Button("Adicionar \n codificação social",key="add_cod",font=(None,15),size=(16,None)),
    sg.Button("Sair",key="sair",font=(None,15),size=(16,None))]
    
    ]
    return sg.Window('Codificação variáveis sociais', layout, finalize=True,grab_anywhere=True,text_justification='center',element_justification='center')
...
def ver_tabela(dataf): # mostra o dataframe selecionado para inserção
    ...
    layout =[
        ...
        [sg.Button("Salvar planilha",key='salvar3'), sg.Input(key='nome_arquivo2',default_text='')]

    ]

    return sg.Window('Tabela de dados dos informantes', layout,finalize=True,grab_anywhere=True,resizable=True)
...
def salvar(valor_input,tela,dataf): # salva a planilha codificada
    ...
    if nome != '':
        n_df.to_excel(f'linguistic/saidas/planilhas/{nome}.xlsx',sheet_name='com códigos sociais')
    else:
        n_df.to_excel('linguistic/saidas/planilhas/planilha_com_cod_soc.xlsx',sheet_name='com códigos sociais')
        
    sg.popup('ARQUIVO SALVO',title='Confirmação')

def cods(valor,coluna): # faz a correspondência entre a as colunas e os códigos definidos
    cod = ''
    r = str(valor)
